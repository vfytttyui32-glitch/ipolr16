...
def product_list(request):
    ...
    filters = Q()
    if search_query:
        filters &= Q(nameicontains=search_query) | Q(descriptionicontains=search_query)
...
def checkout(request):
    """Оформление заказа, генерация чека и отправка на почту"""
    cart, created = Cart.objects.get_or_create(user=request.user)
    ...
    if request.method == 'POST':
        ...
        # Блок try-except перенесен внутрь функции, где создана переменная email
        try:
            email.send()
